...
import { prospectThemeById } from "@/lib/site-design";
...
/** IDs surfaced only after the judge runs the pipeline on the dashboard. */
...
export function getWorkspaceProspects(prospects: Prospect[]): Prospect[] {
  const initialSet = new Set<string>(dashboardPipelineIds);
  const waveSet = new Set<string>(discoveryWaveIds);
  const seedById = new Map(seededProspects.map((p) => [p.id, p]));

  return prospects.filter((p) => {
    if (initialSet.has(p.id)) return true;
    if (!waveSet.has(p.id)) return false;

    const seed = seedById.get(p.id);
    if (!seed) return false;

    return (
      p.updatedAt !== seed.updatedAt ||
      p.agentState !== seed.agentState ||
...
export function buildDashboardWorkspace() {
  ...
  const transcripts = prospects.flatMap((item) => createTranscriptForProspect(item));
